...
export async function getAppreciationApplications(statusFilter?: string) {
  ...
  // This function is intentionally read-only. Loading the admin desk must
  // never approve, reject, recover, or otherwise mutate a candidate. Payment
  // validation belongs to payment completion and explicit admin decisions.
...
export async function updateAppreciationStatus(
  ...
  try {
    ...
    if (finalStatus === "APPROVED") {
      ...
      if (!payment) {
        return { success: false, error: "Cannot approve: no payment record is linked to this application." };
      }
      ...
      // A previous page-load reconciliation could have incorrectly marked a
      // genuine payment FAILED when its old gateway ID was absent. A live
      // PhonePe match is authoritative and safely restores that payment here.
      if (!isAuthorizedAdminIssue && payment.status !== "COMPLETED") {
        await supabase
          .from("payments")
          .update({
            status: "COMPLETED",
            gateway_transaction_id: gatewayCheck?.transactionId || payment.transaction_id,
            failure_reason: null
          })
          .eq("id", payment.id);
      }
    ...
    // 3. Send notification email to candidate safely in background
    try {
      ...
      let emailHtml = `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
          <div style="background-color: #1E60B4; padding: 24px; text-align: center;">
            <img src="https://www.dkffj.org/logo.png" alt="DKFFJ Logo" style="width: 70px; height: 70px; margin-bottom: 12px; display: inline-block;" />
      ...
      if (finalStatus === "APPROVED") {
        emailHtml += `
          ...
          <p>You can verify and download a digital copy of your Certificate of Appreciation anytime from our official portal:</p>
          <div style="margin-top: 20px;">
            <a href="${process.env.NEXT_PUBLIC_APP_URL || 'https://www.dkffj.org'}/track?type=appreciation&id=${encodeURIComponent(app.application_no)}" style="background-color: #15803d; color: #ffffff; padding: 10px 20px; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 13px; display: inline-block;">View & Download Certificate</a>
          </div>
        `;
